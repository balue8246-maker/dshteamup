...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function TeamUpPanel({ onClose }) {
      ...
      const [actionBusy, setActionBusy] = useState(null) // 正在记账/释放/裁决的 id
      // in-flight 去重（线程/账本/锁/审批各自独立）：上一请求未返回时，15s 自动刷新/
      // 手动刷新直接跳过本次，杜绝重叠请求（慢响应 + 周期 tick 不会并发堆积）。
      const inFlightRef = useRef({ threads: false, ledger: false, locks: false, approvals: false })
      ...
      }, [loadThreads, loadLedger, loadLocks, loadApprovals])
      ...
      return ReactDOM.createPortal(
        React.createElement('div', { className: 'tu-panel', role: 'dialog' },
          React.createElement('div', { className: 'tu-panel-head' },
            ...
            React.createElement('button', {
              className: 'tu-btn',
              onClick: tab === 'threads' ? loadThreads : (tab === 'ledger' ? loadLedger : (tab === 'locks' ? loadLocks : loadApprovals)),
              disabled: tab === 'threads' ? threadData.loading : (tab === 'ledger' ? ledgerData.loading : (tab === 'locks' ? locksData.loading : approvalData.loading)),
              title: '刷新',
            }, (tab === 'threads' ? threadData.loading : (tab === 'ledger' ? ledgerData.loading : (tab === 'locks' ? locksData.loading : approvalData.loading))) ? '…' : '刷新'),

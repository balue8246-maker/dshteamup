 ...
 *
 * 面板内容（两个标签页，均 15s 自动刷新 + in-flight 去重，无重叠请求）：
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function TeamUpPanel({ onClose }) {
      ...
      // in-flight 去重（线程/账本各自独立）：上一请求未返回时，15s 自动刷新/
      // 手动刷新直接跳过本次，杜绝重叠请求（慢响应 + 周期 tick 不会并发堆积）。
      const inFlightRef = useRef({ threads: false, ledger: false })
      ...
      // 面板打开期间每 15s 自动刷新两个数据源（与手动刷新同一条 load 路径）
      useEffect(() => {
      ...
      return ReactDOM.createPortal(
        React.createElement('div', { className: 'tu-panel', role: 'dialog' },
          React.createElement('div', { className: 'tu-panel-head' },
            ...
            React.createElement('button', {
              className: 'tu-btn',
              onClick: tab === 'threads' ? loadThreads : loadLedger,
              disabled: tab === 'threads' ? threadData.loading : ledgerData.loading,
              title: '刷新',
            }, tab === 'threads' ? (threadData.loading ? '…' : '刷新') : (ledgerData.loading ? '…' : '刷新')),
            React.createElement('button', { className: 'tu-btn tu-btn-close', onClick: onClose, title: '关闭' }, '×')
          ),
          tab === 'threads' ? threadContent : ledgerContent,

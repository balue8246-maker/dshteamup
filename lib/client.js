...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const STYLE_TEXT = [
      ...
      '.tu-exp-select{max-width:86px;font-size:11px;background:var(--dsw-alias-button-elevated-fill,#2b3242);color:inherit;border:1px solid var(--dsw-alias-border-l2,#3a4256);border-radius:6px;padding:1px 4px}',
    ...
    // 面板=主脑操作台：手动覆盖期望状态
    function setExpectation(threadId, expected) {
      return fetchJson('/teamup/api/expectations/set', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ threadId, expected }),
      })
    }
    ...
    function TeamUpPanel({ onClose }) {
      ...
      // ---- 主脑状态板标签页内容（期望 vs 实际 + 偏差） ----
      const doSetExpectation = useCallback((threadId, expected) => {
        if (!expected || actionBusy) return
        setActionBusy(threadId)
        setExpectation(threadId, expected).then(
          () => {
            setActionBusy(null)
            loadBoard()
          },
      ...
      if (boardData.error) {
        ...
        threadContent = React.createElement('div', { className: 'tu-thread-list' },
          boardData.rows.map((r) => {
            const expLabel = r.expected ? (EXPECTED_LABELS[r.expected] || r.expected) : '未设'
            ...
            return React.createElement('div', {
              ...
              React.createElement('div', { className: 'tu-thread-main' },
                ...
                React.createElement('select', {
                  className: 'tu-exp-select',
                  value: r.expected || '',
                  disabled: actionBusy !== null,
                  onChange: (e) => doSetExpectation(r.threadId, e.target.value),
                  title: '主脑手动覆盖期望状态',
                },
                  React.createElement('option', { value: '' }, '未设'),
                  Object.keys(EXPECTED_LABELS).map((k) => React.createElement('option', { value: k, key: k }, EXPECTED_LABELS[k]))
                )
              ),
              React.createElement('div', { className: 'tu-thread-meta' },
                React.createElement('span', { className: 'tu-exp tu-exp-' + (r.expected || '') }, '期望：', expLabel),

...
export function apply(ctx, config = {}) {
  ...
  function resolveAgentOptions(args) {
    ...
    return {
      ...(args.provider || inherited.provider ? { provider: args.provider || inherited.provider } : {}),
      ...(args.model || inherited.model ? { model: args.model || inherited.model } : {}),
      ...(args.maxTokens || inherited.maxTokens ? { maxTokens: Number(args.maxTokens || inherited.maxTokens) } : {}),
    }
  }
  ...
  ctx.tools.register({
    ...
    async execute(args) {
      try {
        ...
        // 关键：不 join agent preset 的子 agent 工具面会落到"空全局层"（只有插件工具）。
        // 用 dsh 标准姿势 composeFrom 继承主 agent（调用方）的 preset 组合——
        // 让专员线程拥有与主脑一致的完整宿主工具面（bash/read/write/web_search 等）。
        const parentAgent = ctx.agents.currentInitiator() ?? (args.parentSession ? ctx.agents.get(args.parentSession) : undefined)
        const setup = (agentCtx) => {
          try {
            const presets = agentCtx.get('agentPresets')
            if (presets && parentAgent) {
            ...
            } else if (presets && !parentAgent) {
              // 无调用方 agent 时（如 HR 建立流程），挂载默认 preset（若有）
              const id = presets.defaultId
              if (id) void presets.mount(agentCtx, id).catch((e) => console.error(TAG, 'preset mount failed:', e && e.message))
            }
          } catch (error) {
            console.error(TAG, 'preset compose failed:', error && error.message)
          }
        }
  ...
  // 诊断工具：列出指定线程（或调用方 agent）可见的工具面。
  // 用于排查"插件线程工具面缺失"问题（对比主 agent 与插件线程的 tools.view 差异）。
  ctx.tools.register({
    name: 'teamup_debug_tools',
    description: '【TeamUp 诊断】List the tool surface visible to a thread (or the calling agent). 列出某线程/调用方可用的全部工具名，用于排查工具面装配差异。',
    parameters: {
      type: 'object',
      properties: {
        threadId: { type: 'string', description: '目标线程 sessionId；缺省列出调用方 agent 自己的工具面。' },
      },
    },
    output: {
      schema: { type: 'object' },
    ...
    async execute(args) {
      try {
        const target = args.threadId ? ctx.agents.get(args.threadId) : undefined
        const agent = target ?? ctx.agents.currentInitiator()
        const visible = agent && agent.ctx && agent.ctx.tools ? [...agent.ctx.tools.view(agent).visible.keys()] : []
        const own = agent && agent.ctx && agent.ctx.tools ? [...agent.ctx.tools.view(agent.ctx).visible.keys()] : []
        return {
          target: target ? args.threadId : (agent ? agent.id : 'none'),
          count: visible.length,
          tools: visible.sort(),
          note: 'view(agent) 为 agent 视角可见工具；若 count 偏少说明 scope/restrict 装配有差异',
        }
      } catch (error) {
        return { ok: false, error: String((error && error.message) || error) }
      }
    ...
    for (const sessionId of ids) {
      try {
        ...
        const setup = (agentCtx) => {
          try {
            const presets = agentCtx.get('agentPresets')
            const id = presets && presets.defaultId
            if (presets && id) void presets.mount(agentCtx, id).catch((e) => console.error(TAG, 'resume preset mount failed:', e && e.message))
          } catch (error) {
            console.error(TAG, 'resume preset compose failed:', error && error.message)
          }
        }
  ...
  // 按 1s/3s/10s 阶梯重试，直到全部线程恢复或确认无法恢复。
  ;[0, 1000, 3000, 10000].forEach((delay, index) => {
    setTimeout(() => {
      rehydrateThreads().then((n) => {
        if (n > 0 || index === 3) console.log(TAG, `rehydration pass ${index + 1}: restored ${n}/${Object.keys(loadRegistry()).length}`)

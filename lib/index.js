...
export function apply(ctx, config = {}) {
  ...
  ctx.tools.register({
    ...
    description:
      '【TeamUp 状态板 - 期望状态】Main brain sets the EXPECTED state of a thread ' +
      '("who should be doing what"). expected ∈ working | idle | paused | awaiting-return | done. ' +
      '自动流转为主（派工→working、暂停/恢复→paused/working、验收→done），本工具为手动覆盖；' +
      ...
      try {
        ...
        if (req.method === 'POST') {
          ...
          if (rest === '/expectations/set') {
            // 面板=主脑操作台：手动覆盖期望状态（与 teamup_set_expectation 同路径）

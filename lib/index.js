...
export function apply(ctx, config = {}) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (req.method === 'POST') {
          if (rest !== '/ledger/submit' && rest !== '/ledger/confirm' && rest !== '/locks/claim' && rest !== '/locks/release') {
            writeJson(res, 404, { ok: false, error: 'not found' })
            return
          }
  ...
  // ---- 诊断工具：唤醒链路测试（临时，验证后可删） ----
  // 对目标线程（缺省=调用方）followup 一条测试消息，随后检查：
  //  target.status（是否被唤醒）、inbox 是否有 pending、事件日志是否产生 user/message。
  // 用于定位 REQ-03：worker 回报不进主脑对话的断链环节。
  ctx.tools.register({
    name: 'teamup_debug_wake',
    description: '【TeamUp 诊断】向目标线程 followup 一条测试消息，检查唤醒链路（status/inbox/user-message 事件）。用于排查 worker 回报不进主脑对话的问题。',
    parameters: {
      type: 'object',
      properties: {
        threadId: { type: 'string', description: '目标线程 sessionId；缺省 = 调用方自己。' },
        message: { type: 'string', description: '测试消息内容，缺省"【唤醒测试】你在吗？"。' },
      },
    },
    output: {
      schema: { type: 'object' },
    ...
    async execute(args) {
      try {
        const target = args.threadId ? ctx.agents.get(args.threadId) : ctx.agents.currentInitiator()
        if (!target) return { ok: false, error: `thread not found: ${args.threadId || 'self'}` }
        const before = {
          status: target.status,
          inboxPending: target.inbox ? target.inbox.hasPending : null,
          lastEventSeq: target.session.seq,
        }
        const userMessage = makeUserMessage(args.message || '【唤醒测试】你在吗？')
        let followupResult = null
        try {
          target.followup(userMessage)
          followupResult = 'followup called'
        } catch (error) {
          followupResult = 'followup threw: ' + errMsg(error)
        }
        // 短暂等待 driver 处理（如果有 wakeup）
        ...
        // 找 user/message 事件（消息是否注入 model surface）
        let userMessageEvent = null
        const events = target.session.events
        for (let i = events.length - 1; i >= 0; i--) {
          const event = events[i]
          if (event.type === 'user/message' && event.data && event.data.id === userMessage.id) {
            userMessageEvent = { seq: event.seq, type: event.type }
            break
          }
          if (event.type === 'agent/inbox/spliced' && event.data && Array.isArray(event.data.inserted) && event.data.inserted.some((m) => m && m.id === userMessage.id)) {
            userMessageEvent = { seq: event.seq, type: 'agent/inbox/spliced' }
            break
          }
        }
        return {
          ...
          before,
          after,
          wakeDetected: before.status !== after.status,
          messageEvent: userMessageEvent,
          followupResult,
          conclusion: userMessageEvent
            ? (userMessageEvent.type === 'user/message' ? '消息已注入 model surface（进对话）' : '消息进了 inbox 但未转 user/message（未进对话）')
            : '消息完全没找到（followup 未生效？）',
        }
      } catch (error) {
        return { ok: false, error: errMsg(error) }
      }

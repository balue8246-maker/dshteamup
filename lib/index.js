...
// ---- 文件锁（协议层写安全，MVP） ----
// 锁状态与 registry 同目录落盘：~/.dsh/profiles/web/dshteamup/locks.json
// （path 归一化为绝对路径；claimedBy 记录持有者 sessionId/标题）。
// 注意：dsh 无法硬拦截 worker 的 bash/write 工具，锁是协议层强制——
// 靠【写前查锁】纪律注入 + 面板可见性 + 冲突检测（409/403）兜底。
const LOCKS_PATH = resolve(os.homedir(), '.dsh', 'profiles', 'web', 'dshteamup', 'locks.json')

// ---- 主脑状态板（期望状态，声明非事实） ----
// expectations.json：threadId → { expected, taskId?, note?, updatedAt, setBy }
// expected ∈ working | idle | paused | awaiting-return | done。
// 自动流转：send_message 识别派工（任务编号）→ working；暂停/恢复指令 →
// paused/working；ledger confirm（验收）→ done；手动 teamup_set_expectation /
// 面板 POST 覆盖为辅。偏差只告警不自动纠正。
const EXPECTATIONS_PATH = resolve(os.homedir(), '.dsh', 'profiles', 'web', 'dshteamup', 'expectations.json')
const EXPECTED_STATES = ['working', 'idle', 'paused', 'awaiting-return', 'done']
// 偏差阈值：期望 working 但实际 idle 且最后活动距今超过该秒数 → 🔴 疑似中断
const EXPECTED_STALL_SECONDS = 3 * 60
...
8. 【写前查锁】写/改文件前必须 teamup_list_locks 检查：目标文件未被自己持有锁时，
   先用 teamup_claim_file 拿锁再写（写后 teamup_release_file 释放）；若锁被他人持有
   （冲突 409），绝不硬写，回报主脑协调（主脑可强制释放）。
9. 【关键操作审批】涉及写所有权冲突边界/危险操作（危险命令、大范围删除、跨边界
   动作）时，先 teamup_ask_approval 发起审批，等待主脑裁决：allowed-once 才继续；
   rejected/cancelled 立即停止并回报主脑。`
...
export function apply(ctx, config = {}) {
  ...
  // 锁文件路径（config 可覆盖，冒烟用 throwaway 路径）
  const locksPath = config.locksPath ?? LOCKS_PATH
  ...
  // ---- 文件锁存储（协议层写安全） ----
  // locks.json：{ [normalizedPath]: { path, claimedBy: {kind, sessionId, title}, claimedAt, reason } }
  // 身份模型：工具调用方 = currentInitiator（agent）；面板调用（无 actor）= 'panel'，
  // 视同主脑身份（面板是主脑/用户操作台）。主脑（resolveMainBrainThreadId）可强制
  // 释放任意锁。锁是协议层强制，非安全边界——README 与纪律已注明。
  function loadLocks() {
    try {
      if (!existsSync(locksPath)) return {}
      return JSON.parse(readFileSync(locksPath, 'utf8')) || {}
    } catch (error) {
      console.error(TAG, 'failed to load locks:', error && error.message)
      return {}
    }
  }
  ...
  function saveLocks(locks) {
    try {
      ...
      console.error(TAG, 'failed to save locks:', error && error.message)
    }
  }

  function lockList() {
    const locks = loadLocks()
    return Object.values(locks).sort((a, b) => String(a.path).localeCompare(String(b.path)))
  }

  // path 归一化：必须是绝对路径（相对路径按 defaultTeamDir 解析），
  // 长度受限、禁控制字符——白名单式纵深防御（同 taskId 校验风格）。
  function normalizeLockPath(rawPath) {
    const p = String(rawPath || '').trim()
    if (!p || p.length > 4096) return null
    if (/[\x00-\x1f\x7f]/.test(p)) return null
    const absolute = p.startsWith('/') ? p : resolve(defaultTeamDir, p)
    return absolute
  }

  // actor 是否主脑/面板（可强制释放）：工具层传 agent sessionId，
  // 面板调用不传 actor（'panel'）——面板=主脑操作台，拥有主脑权限。
  function isMainBrainActor(actorSessionId) {
    if (!actorSessionId) return true // panel 调用（无 actor）视同主脑
    return actorSessionId === resolveMainBrainThreadId()
  }

  function isLockHolder(lock, actorSessionId) {
    return !!actorSessionId && lock.claimedBy && lock.claimedBy.sessionId === actorSessionId
  }

  // 内部 claim/release 实现（工具层与 HTTP 端点共用）。
  // 返回 { ok, lock?, holder?, error?, status? }；status 供端点映射 HTTP 码。
  function claimLock(path, reason, actorSessionId) {
    const locks = loadLocks()
    ...
    if (existing) {
      ...
      const owned = isLockHolder(existing, actorSessionId)
      return {
        ok: false,
        status: 409,
        error: owned ? 'already claimed by you' : 'locked by another holder',
        holder,
      }
    }
    const lock = {
      path,
      claimedBy: {
        kind: actorSessionId ? 'agent' : 'panel',
        sessionId: actorSessionId || 'panel',
        title: actorSessionId ? (loadRegistry()[actorSessionId] && loadRegistry()[actorSessionId].title) || null : 'web-panel',
      },
    ...
    locks[path] = lock
    saveLocks(locks)
    console.log(TAG, `lock claimed: ${path} by ${lock.claimedBy.sessionId}`)
    return { ok: true, status: 200, lock }
  }

  function releaseLock(path, actorSessionId) {
    const locks = loadLocks()
    const existing = locks[path]
    if (!existing) return { ok: false, status: 404, error: 'lock not found' }
    if (!isLockHolder(existing, actorSessionId) && !isMainBrainActor(actorSessionId)) {
      return {
        ok: false,
        status: 403,
        error: 'only the holder or the main brain can release this lock',
        holder: {
          path: existing.path,
          claimedBy: existing.claimedBy,
          claimedAt: existing.claimedAt,
    ...
    delete locks[path]
    saveLocks(locks)
    console.log(TAG, `lock released: ${path} by ${actorSessionId || 'panel'}`)
    return { ok: true, status: 200, released: { path, claimedBy: existing.claimedBy } }
  }

  // ---- 审批门（复用原生 ctx.approval + prepend answerer 转发） ----
  // 混合路径（R2 结论）：worker 经 teamup_ask_approval 走 ctx.approval.request()
  // （原生审计 approval/asked+decided 落 worker session）；本插件注册
  // prepend 的 "approval/request" answerer（先于 host-apiproxy 的浏览器 answerer），
  // 只拦截本插件创建的 worker 的审批——登记 pending、等主脑（面板或
  // teamup_decide_approval）裁决后 resolve；非 worker 审批调用 next() 透传，
  // 保持原生浏览器审批流不变。
  // U1 结论：主脑无法经 /api/respond 程序化应答（rpcId 服务端铸造、只走浏览器
  // mux 通道，agent 无渠道获取）——故裁决面为面板 + 工具，不走 respond。
  const pendingApprovals = new Map() // approvalId -> record（含 settle）
  const decidedApprovals = []        // 最近已裁决（ring buffer，供面板展示）
  const APPROVAL_DECIDED_LIMIT = 50

  function approvalPendingList() {
    const out = []
    for (const record of pendingApprovals.values()) {
      out.push({
        approvalId: record.approvalId,
        workerSessionId: record.workerSessionId,
        workerTitle: record.workerTitle,
        toolName: record.toolName,
        reason: record.reason,
        askedAt: record.askedAt,
      })
    }
    out.sort((a, b) => String(a.askedAt).localeCompare(String(b.askedAt)))
    return out
  }

  function decideApproval(approvalId, outcome) {
    const record = pendingApprovals.get(approvalId)
    if (!record) return { ok: false, status: 404, error: 'approval not pending' }
    if (outcome !== 'allowed-once' && outcome !== 'rejected') {
      return { ok: false, status: 400, error: 'invalid outcome (allowed-once|rejected)' }
    }
    record.settle(outcome)
    return { ok: true, status: 200, approvalId, outcome, workerSessionId: record.workerSessionId }
  }

  // prepend answerer：本插件 worker 的审批在此拦截；其余透传给 host-apiproxy。
  ctx.on('approval/request', (req, next) => {
    const agent = req && req.agent
    if (!agent || !agent.session) return next()
    if (!handles.has(agent.id)) return next() // 只拦截本插件创建的线程
    const events = agent.session.events
    ...
    const record = {
      approvalId,
      workerSessionId: agent.id,
      workerTitle: (loadRegistry()[agent.id] && loadRegistry()[agent.id].title) || null,
    ...
    const outcomePromise = new Promise((resolve) => {
      const onAbort = () => settleOutcome('cancelled')
      const settleOutcome = (outcome) => {
        if (record.settled) return
        record.settled = true
        req.signal && typeof req.signal.removeEventListener === 'function' && req.signal.removeEventListener('abort', onAbort)
        pendingApprovals.delete(approvalId)
        decidedApprovals.unshift({
          approvalId,
          workerSessionId: agent.id,
          toolName: req.toolName,
          reason: req.reason ?? null,
        ...
        if (decidedApprovals.length > APPROVAL_DECIDED_LIMIT) decidedApprovals.length = APPROVAL_DECIDED_LIMIT
        console.log(TAG, `approval decided: ${approvalId} -> ${outcome}`)
        resolve(outcome)
      }
      record.settle = settleOutcome
      if (req.signal && typeof req.signal.addEventListener === 'function') {
        req.signal.addEventListener('abort', onAbort, { once: true })
      }
    })
    pendingApprovals.set(approvalId, record)
    console.log(TAG, `approval requested: ${approvalId} by ${agent.id} (${req.toolName})`)
    return outcomePromise
  }, { prepend: true })

  // ---- 主脑状态板（期望状态存储 + 偏差检测） ----
  const expectationsPath = config.expectationsPath ?? EXPECTATIONS_PATH

  function loadExpectations() {
    try {
      if (!existsSync(expectationsPath)) return {}
      return JSON.parse(readFileSync(expectationsPath, 'utf8')) || {}
    } catch (error) {
      console.error(TAG, 'failed to load expectations:', error && error.message)
      return {}
    }
  }
  ...
  function saveExpectations(expectations) {
    try {
      mkdirSync(dirname(expectationsPath), { recursive: true })
      writeFileSync(expectationsPath, JSON.stringify(expectations, null, 2))
    } catch (error) {
      console.error(TAG, 'failed to save expectations:', error && error.message)
    }
  }

  // 写入/更新一条期望状态（threadId 不存在也允许——可为未来线程预设）。
  function setExpectation(threadId, expected, extra = {}) {
    if (!EXPECTED_STATES.includes(expected)) return { ok: false, status: 400, error: `invalid expected state (${EXPECTED_STATES.join('|')})` }
    const threadIdStr = String(threadId || '').trim()
    if (!threadIdStr) return { ok: false, status: 400, error: 'threadId is required' }
    const expectations = loadExpectations()
    const record = {
      expected,
      ...(extra.taskId ? { taskId: String(extra.taskId).trim() } : {}),
      ...(extra.note ? { note: String(extra.note).trim() } : {}),
      updatedAt: new Date().toISOString(),
      setBy: extra.setBy || 'manual',
    }
    expectations[threadIdStr] = record
    saveExpectations(expectations)
    console.log(TAG, `expectation set: ${threadIdStr} -> ${expected}${record.taskId ? ' (task ' + record.taskId + ')' : ''}`)
    return { ok: true, status: 200, record }
  }

  // ledger confirm（验收）→ 把对应任务绑定的线程期望置为 done。
  function expectDoneByTask(taskId) {
    if (!taskId) return
    const expectations = loadExpectations()
    let changed = false
    for (const [threadId, record] of Object.entries(expectations)) {
      if (record.taskId === taskId && record.expected !== 'done') {
        record.expected = 'done'
        record.updatedAt = new Date().toISOString()
        record.setBy = 'auto:ledger-confirm'
        changed = true
      }
    }
    if (changed) saveExpectations(expectations)
  }

  // send_message 消息的期望状态自动识别（自动为主）：
  //  - 派工（含「任务编号：」）→ working（带任务编号）
  //  - 暂停/恢复指令 → paused / working
  // 返回 { matched, expected, taskId }；不匹配返回 { matched: false }（不动期望）。
  function expectationIntentOf(message) {
    const text = String(message || '')
    const taskMatch = /任务编号[：:]\s*([A-Za-z0-9._-]+)/.exec(text)
    if (taskMatch) {
      return { matched: true, expected: 'working', taskId: taskMatch[1] }
    }
    if (/暂停|pause/i.test(text)) return { matched: true, expected: 'paused' }
    if (/恢复|resume/i.test(text)) return { matched: true, expected: 'working' }
    return { matched: false }
  }

  // 会话最后活动时间（events 最后一个带 ts 的事件；无则 null）。
  function lastActivityAt(agent) {
    try {
      const events = agent.session.events
      for (let i = events.length - 1; i >= 0; i--) {
        const ts = events[i] && events[i].ts
  ...
  // 偏差检测：期望 vs 实际。
  //  - 期望 working 且实际 idle 且最后活动距今 > 3 分钟 → 🔴 suspected-stall 疑似中断
  //  - 期望 idle/paused/done 但实际 running → 🟡 未派工活动 / 暂停中却在活动 / 已验收仍在活动
  function deviationOf(exp, actual, stalenessSeconds) {
    if (!exp) return null
    const working = exp.expected === 'working'
    if (working && actual === 'idle') {
      if (stalenessSeconds !== null && stalenessSeconds > EXPECTED_STALL_SECONDS) {
        return { level: 'red', code: 'suspected-stall', label: '疑似中断' }
      }
      return null // 刚派工/热身期不算偏差
    }
    if (exp.expected === 'idle' && actual === 'running') {
      return { level: 'yellow', code: 'unassigned-activity', label: '未派工活动' }
    }
    if (exp.expected === 'paused' && actual === 'running') {
      return { level: 'yellow', code: 'paused-but-running', label: '暂停中却在活动' }
    }
    if (exp.expected === 'done' && actual === 'running') {
      return { level: 'yellow', code: 'done-but-running', label: '已验收仍在活动' }
    }
    return null
  }

  // 状态板：线程 ×（期望 / 实际 / 偏差 / staleness）。
  function computeStatusBoard() {
    ...
    return ctx.agents.list().map((agent) => {
      const header = agent.session.header
      const meta = registry[agent.id] || {}
      const exp = expectations[agent.id] || null
      const lastAt = lastActivityAt(agent)
      const stalenessSeconds = lastAt === null ? null : Math.max(0, Math.floor((now - lastAt) / 1000))
      return {
        threadId: agent.id,
        title: meta.title ?? null,
        teamName: meta.teamName ?? null,
        status: agent.status,
        expected: exp ? exp.expected : null,
        taskId: exp ? exp.taskId ?? null : null,
        note: exp ? exp.note ?? null : null,
        expectedUpdatedAt: exp ? exp.updatedAt : null,
        expectedSetBy: exp ? exp.setBy : null,
        lastActivityAt: lastAt === null ? null : new Date(lastAt).toISOString(),
        stalenessSeconds,
        deviation: deviationOf(exp, agent.status, stalenessSeconds),
      }
    })
  }
  ...
  ctx.tools.register({
    ...
    async execute(args) {
      try {
        ...
        // 主脑状态板自动流转：派工（任务编号）→ working；暂停/恢复 → paused/working
        const intent = expectationIntentOf(args.message)
        if (intent.matched) {
          setExpectation(args.threadId, intent.expected, {
            taskId: intent.taskId,
            note: intent.taskId ? '派工 ' + intent.taskId : (intent.expected === 'paused' ? '暂停指令' : '恢复指令'),
            setBy: 'auto:send_message',
          })
        }
        return {
          ok: true,
  ...
  ctx.tools.register({
    ...
    async execute(args) {
      try {
        ...
        // REQ-04：dispose 只清内存（agent 实例），磁盘 session 数据保留 →
        // 重启后 dsh session 层重新挂载为无主幽灵。调 archiveSession 写入
        // workspace.json 的 archivedSessionIds——侧栏隐藏、重启不复活。
        try {
          const wr = ctx.workspaceRegistry
          if (wr) await wr.archiveSession(args.threadId)
        } catch (error) {
          console.error(TAG, `archiveSession failed for ${args.threadId}:`, error && error.message)
        }
  ...
  // ---- 文件锁工具（协议层写安全；worker/主脑可用） ----
  // 纪律（WORKER_RULES 第 8 条）：写/改文件前先 teamup_list_locks 检查，
  // 未持有锁先 teamup_claim_file，冲突绝不硬写、回报主脑协调。

  ctx.tools.register({
    name: 'teamup_claim_file',
    description:
      '【TeamUp 文件锁 - 占锁】Claim exclusive write ownership of a file path before writing it. ' +
      '写/改文件前必须先拿锁（见纪律第 8 条）。冲突（被他人持有）返回 409 与持有者信息，' +
      '此时绝不硬写，回报主脑协调。path 需为绝对路径（相对路径按主进程 cwd 解析）。' +
      '写完后用 teamup_release_file 释放。',
    parameters: {
      type: 'object',
      properties: {
        path: { type: 'string', description: '目标文件绝对路径（如 /Users/acegent/Documents/DS Projects/src/a.js）。' },
        reason: { type: 'string', description: '占锁原因（可选，如"实现 W3 文件锁"），面板可见。' },
      },
      required: ['path'],
    },
    output: {
    ...
    async execute(args) {
      try {
        const actor = (() => {
          try {
            const initiator = ctx.agents.currentInitiator()
            return initiator && initiator.id ? initiator.id : null
          } catch {
            return null
          }
        })()
        const path = normalizeLockPath(args.path)
        if (!path) return { ok: false, error: 'invalid path (must be an absolute path, <=4096 chars, no control characters)' }
        const result = claimLock(path, args.reason, actor)
        if (!result.ok) {
          return {
            ok: false,
            error: result.error,
        ...
        return { ok: true, lock: result.lock, path }
      } catch (error) {
        return { ok: false, error: errMsg(error) }
      }
    },
  ...
  ctx.tools.register({
    name: 'teamup_release_file',
    description:
      '【TeamUp 文件锁 - 释放】Release a file lock held by you (or by anyone, when called by the main brain). ' +
      '仅持有者可释放，否则 403；主脑可强制释放。写完文件后必须释放，避免锁堆积。',
    parameters: {
      type: 'object',
      properties: {
        path: { type: 'string', description: '目标文件绝对路径（与 claim 时一致）。' },
      },
      required: ['path'],
    },
    output: {
    ...
    async execute(args) {
      try {
        ...
        const result = releaseLock(path, actor)
        if (!result.ok) {
          return { ok: false, error: result.error, status: result.status, holder: result.holder ?? null, path }
        }
        return { ok: true, path, released: result.released }
      } catch (error) {
        return { ok: false, error: errMsg(error) }
      }
    },
  ...
  ctx.tools.register({
    name: 'teamup_list_locks',
    description:
      '【TeamUp 文件锁 - 查锁】List all current file locks (path / holder / claimedAt / reason). ' +
      '写/改文件前必须先调用本工具检查（纪律第 8 条）：目标文件未被自己持锁时先 claim；' +
      '被他人持有时绝不硬写，回报主脑协调。',
    parameters: {
      type: 'object',
      properties: {},
  ...
  // ---- 审批门工具（纪律第 9 条：关键操作先审批） ----

  ctx.tools.register({
    name: 'teamup_ask_approval',
    description:
      '【TeamUp 审批 - 发起】Ask the main brain to approve a critical action before doing it. ' +
      '涉及写所有权冲突边界/危险操作（危险命令、大范围删除、跨边界动作）时必须先发起审批' +
      '（纪律第 9 条），等待主脑裁决：返回 outcome=allowed-once 才继续；' +
      'rejected/cancelled 立即停止并回报主脑。审批请求会出现在主脑面板「审批中心」，' +
      '主脑批准/拒绝后结果回传本调用。全程落会话审计（approval/asked + approval/decided）。',
    parameters: {
      type: 'object',
      properties: {
        action: { type: 'string', description: '要执行的危险/关键操作（简短描述，面板可见）。' },
        reason: { type: 'string', description: '发起审批的原因/理由（可选）。' },
      },
      required: ['action'],
    },
    output: {
    ...
    async execute(args) {
      try {
        ...
        if (!approval) return { ok: false, error: 'approval service unavailable (fail closed)' }
        const initiator = ctx.agents.currentInitiator()
        if (!initiator) return { ok: false, error: 'no initiating agent' }
        const action = String(args.action || '').trim()
        const reason = String(args.reason || '').trim()
        if (!action) return { ok: false, error: 'action is required' }
        const outcome = await approval.request({
          agent: initiator,
          toolName: 'teamup_ask_approval',
          reason: `[${action}]${reason ? ' ' + reason : ''}`,
        })
        return { ok: true, action, reason, outcome, approved: outcome === 'allowed-once' }
      } catch (error) {
        return { ok: false, error: errMsg(error) }
      }
    },
  ...
  ctx.tools.register({
    name: 'teamup_decide_approval',
    description:
      '【TeamUp 审批 - 裁决】Main brain decides one pending approval (approve or reject). ' +
      '裁决后结果回传发起审批的 worker（allowed-once 继续 / rejected 停止），' +
      '并落会话审计。也可在面板「审批中心」点击批准/拒绝（同一裁决路径）。',
    parameters: {
      type: 'object',
      properties: {
        approvalId: { type: 'string', description: '待裁决审批 id（teamup_list_approvals 或面板可见）。' },
        approved: { type: 'boolean', description: 'true=批准（allowed-once）；false=拒绝（rejected）。' },
      },
    ...
    async execute(args) {
      try {
        ...
        const result = decideApproval(approvalId, args.approved === false ? 'rejected' : 'allowed-once')
        if (!result.ok) return { ok: false, error: result.error, status: result.status }
        return { ok: true, approvalId, outcome: result.outcome, workerSessionId: result.workerSessionId }
      } catch (error) {
        return { ok: false, error: errMsg(error) }
      }
    },
  ...
  ctx.tools.register({
    name: 'teamup_list_approvals',
    description:
      '【TeamUp 审批 - 列表】List pending and recently decided approval requests ' +
      '(approvalId / worker / action / reason / askedAt / outcome). ' +
      '主脑可据此用 teamup_decide_approval 裁决；面板「审批中心」显示同一数据。',
    parameters: {
      type: 'object',
      properties: {},
    },
    ...
    async execute() {
      try {
        return { ok: true, pendingCount: pendingApprovals.size, pending: approvalPendingList(), decided: decidedApprovals.slice(0, 20) }
      } catch (error) {
        return { ok: false, error: errMsg(error) }
      }
    },
  ...
  ctx.tools.register({
    name: 'teamup_set_expectation',
    description:
      '【TeamUp 状态板 - 期望状态·内部兜底】Manually override the EXPECTED state of a thread. ' +
      '【仅系统内部兜底/调试用】正常情况期望状态由派工（→working）、验收 confirm（→done）、' +
      '暂停/恢复指令（→paused/working）自动流转，无需手动干预；本工具只应在自动流转失效或' +
      '异常恢复时由主脑调用。expected ∈ working | idle | paused | awaiting-return | done。' +
      '期望状态是声明非事实，只用于状态板偏差告警，不自动纠正线程行为。',
    parameters: {
      type: 'object',
      properties: {
        threadId: { type: 'string', description: '目标线程 sessionId。' },
        expected: { type: 'string', enum: EXPECTED_STATES, description: '期望状态：working（应干活）/ idle（应空闲）/ paused（应暂停）/ awaiting-return（应等待回报）/ done（应已完成）。' },
        taskId: { type: 'string', description: '任务编号（可选，派工关联用，验收后自动 done）。' },
        note: { type: 'string', description: '备注（可选，面板可见）。' },
      },
      required: ['threadId', 'expected'],
    },
    output: {
      schema: { type: 'object' },
      render: (_args, value) => [{ type: 'text', text: JSON.stringify(value, null, 2) }],
    ...
    async execute(args) {
      try {
        const result = setExpectation(args.threadId, args.expected, {
          taskId: args.taskId,
          note: args.note,
          setBy: 'tool:main-brain',
        })
        if (!result.ok) return { ok: false, error: result.error, status: result.status }
        return { ok: true, threadId: String(args.threadId).trim(), record: result.record }
      } catch (error) {
        return { ok: false, error: errMsg(error) }
      }
    },
  ...
  ctx.tools.register({
    name: 'teamup_list_expectations',
    description:
      '【TeamUp 状态板 - 查看】List the status board: per-thread EXPECTED state vs ACTUAL ' +
      'state (idle/running + last-activity staleness) + deviation flags. ' +
      '偏差：期望 working 但 idle 超阈值→🔴 疑似中断；期望 idle/paused/done 但 running→🟡。',
    parameters: {
      type: 'object',
      properties: {},
    },
      ...
      try {
        if (req.method === 'GET') {
          if (rest === '' || rest === '/' || rest === '/threads') {
            ...
            const locks = lockList()
            writeJson(res, 200, { ok: true, count: locks.length, locks })
          } else if (rest === '/approvals') {
            // 审批中心数据：pending（等待主脑裁决）+ 最近已裁决
            writeJson(res, 200, {
              ok: true,
              pendingCount: pendingApprovals.size,
              pending: approvalPendingList(),
              decided: decidedApprovals.slice(0, 20),
            })
          } else if (rest === '/expectations') {
            // 主脑状态板：线程 ×（期望/实际/偏差/staleness）
            writeJson(res, 200, { ok: true, rows: computeStatusBoard() })
          } else {
        ...
        if (req.method === 'POST') {
          if (rest !== '/ledger/submit' && rest !== '/ledger/confirm' && rest !== '/locks/claim' && rest !== '/locks/release' && rest !== '/approval/decide' && rest !== '/expectations/set') {
            writeJson(res, 404, { ok: false, error: 'not found' })
            return
          }
          const body = await readJsonBody(req)
          if (rest === '/expectations/set') {
            // 【内部兜底】手动覆盖期望状态（与 teamup_set_expectation 同路径）。
            // 正常情况期望状态由派工/验收/暂停/恢复自动流转，本端点仅供内部兜底/调试；
            // UI（状态板）已不暴露手动设置。
            const result = setExpectation(body.threadId, body.expected, {
              taskId: body.taskId,
              note: body.note,
              setBy: 'panel',
            })
            ...
            writeJson(res, 200, { ok: true, threadId: String(body.threadId).trim(), record: result.record })
            return
          }
          if (rest === '/approval/decide') {
            // 面板=主脑操作台：批准/拒绝审批（与 teamup_decide_approval 同路径）
            const approvalId = String(body.approvalId || '').trim()
            if (!approvalId) {
              writeJson(res, 400, { ok: false, error: 'approvalId is required' })
              return
            }
            const result = decideApproval(approvalId, body.outcome === 'rejected' ? 'rejected' : 'allowed-once')
            if (!result.ok) {
              writeJson(res, result.status, { ok: false, error: result.error })
              return
            }
            writeJson(res, 200, { ok: true, approvalId, outcome: result.outcome, workerSessionId: result.workerSessionId })
            return
          }
          if (rest === '/locks/claim' || rest === '/locks/release') {
            // 面板=主脑操作台：HTTP 调用一律按 'panel'（主脑）身份，可强制释放。
            // 工具层（worker 身份校验）走内部 claimLock/releaseLock，不经 HTTP。
            const path = normalizeLockPath(body.path)
            if (!path) {
              writeJson(res, 400, { ok: false, error: 'invalid path (absolute path, <=4096 chars, no control characters)' })
              return
            }
            if (rest === '/locks/claim') {
              const result = claimLock(path, body.reason, null)
              if (!result.ok) {
                writeJson(res, result.status, { ok: false, error: result.error, holder: result.holder ?? null })
                return
              }
              writeJson(res, 200, { ok: true, lock: result.lock })
            } else {
              const result = releaseLock(path, null)
              if (!result.ok) {
                writeJson(res, result.status, { ok: false, error: result.error, holder: result.holder ?? null })
                return
              }
              writeJson(res, 200, { ok: true, released: result.released })
            }
            return
          }
          ensureLedgerInitialized()
          ...
          if (rest === '/ledger/submit') {
            ...
            // 状态板自动流转：验收确认 → 对应任务的线程期望置 done
            expectDoneByTask(taskId)
